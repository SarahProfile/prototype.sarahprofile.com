...
add_filter( 'woocommerce_product_data_tabs', 'elex_ppct_custom_product_tab', 10, 1 );
add_action( 'woocommerce_admin_field_elex_ppct_custom_fields_suffix_checkbox', 'elex_ppct_custom_field_suffix' );

function elex_ppct_custom_field_suffix( $value ) {
	global $post;
	$suffix_checkbox = get_post_meta( $post->ID, 'elex_ppct_custom_fields_suffix_checkbox', true );
	?>
	<tr valign="top">
...
function elex_ppct_custom_product_tab( $default_tabs ) {
...
add_action( 'woocommerce_product_data_panels', 'elex_ppct_custom_tab_data' );
function elex_ppct_custom_tab_data() {
	global $post;
	...
	$args     = array(
		...
		'desc_tip' => true,
		'value'       => ( get_post_meta( $post->ID, 'elex_ppct_custom_fields_checkbox', true ) ) ? get_post_meta( $post->ID, 'elex_ppct_custom_fields_checkbox', true ) : 'no',
	);
...
<?php
	...
	$args     = array(
		...
		'desc_tip'    => true,
		'value'       => ( get_post_meta( $post->ID, 'elex_ppct_custom_fields_prefix_checkbox', true ) ) ? get_post_meta( $post->ID, 'elex_ppct_custom_fields_prefix_checkbox', true ) : 'no',
	);
	woocommerce_wp_checkbox( $args );
	$args = array(
		...
		'value'       => ( get_post_meta( $post->ID, 'elex_ppct_custom_fields_prefix', true ) ) ? get_post_meta( $post->ID, 'elex_ppct_custom_fields_prefix', true ) : ' ',
	);
	...
	$args = array(
		...
		'value'       => ( get_post_meta( $post->ID, 'elex_ppct_custom_fields_suffix', true ) ) ? get_post_meta( $post->ID, 'elex_ppct_custom_fields_suffix', true ) : ' ',
	);
	woocommerce_wp_textarea_input( $args );
	...
	$args = array(
		...
		'desc_tip' => true,
		'value'       => ( get_post_meta( $post->ID, 'elex_ppct_discount_type', true ) ) ? get_post_meta( $post->ID, 'elex_ppct_discount_type', true ) : ' ',
	);
	...
	$args = array(
		...
		'value'       => ( get_post_meta( $post->ID, 'elex_ppct_discount_amount', true ) ) ? get_post_meta( $post->ID, 'elex_ppct_discount_amount', true ) : ' ',
	);
...
add_action( 'woocommerce_process_product_meta', 'elex_ppct_save_fields', 10, 1 );

function elex_ppct_save_fields( $post_id ) {
	...
	update_post_meta( $post_id, 'elex_ppct_discount_type', $select_field2 );
	...
	update_post_meta( $post_id, 'elex_ppct_discount_amount', $text_field4 );
}

add_action( 'woocommerce_product_after_variable_attributes', 'elex_ppct_add_custom_field_to_variations', 10, 3 );
function elex_ppct_add_custom_field_to_variations( $loop, $variation_data, $variation ) {
	global $post;
	echo"<h2 style='text-align:center; font-size:20px;'>Product Price Custom Text & Discount For This Variation</h2>";
	$use_custom_text_plugin = get_post_meta( $variation->ID, 'elex_ppct_variation_use_custom_text_plugin', true );
	$use_prefix = get_post_meta( $variation->ID, 'elex_ppct_variation_use_prefix_post_meta', true );
	$use_suffix = get_post_meta( $variation->ID, 'elex_ppct_variation_use_suffix_post_meta', true );
	$use_discount = get_post_meta( $variation->ID, 'elex_ppct_variation_use_discount_post_meta', true );
	?>
...
<?php
	...
	woocommerce_wp_textarea_input(
		array(
			...
			'value'       => get_post_meta( $variation->ID, 'elex_ppct_variation_add_prefix', true ),
		)
	);
	woocommerce_wp_textarea_input(
		array(
			...
			'value'       => get_post_meta( $variation->ID, 'elex_ppct_variation_add_suffix', true ),
		)
	);
	woocommerce_wp_select(
		array(
			...
			'value'       => get_post_meta( $variation->ID, 'elex_ppct_discount_type', true ),
		)
	);
	woocommerce_wp_text_input(
		array(
			...
			'value'             => get_post_meta( $variation->ID, 'elex_ppct_discount_amount', true ),
		)
	);
?>
	<script>
...
add_action( 'woocommerce_save_product_variation', 'elex_ppct_save_custom_field_variations', 10, 2 );
function elex_ppct_save_custom_field_variations( $post_id ) {
	...
	update_post_meta( $post_id, 'elex_ppct_variation_use_custom_text_plugin', $use_custon_text_plugin );

	$use_prefix = isset( $_POST['elex_ppct_variation_use_prefix'][ $post_id ] ) ? 'yes' : 'no';
	update_post_meta( $post_id, 'elex_ppct_variation_use_prefix_post_meta', $use_prefix );

	$use_suffix = isset( $_POST['elex_ppct_variation_use_suffix'][ $post_id ] ) ? 'yes' : 'no';
	update_post_meta( $post_id, 'elex_ppct_variation_use_suffix_post_meta', $use_suffix );

	$use_discount = isset( $_POST['elex_ppct_variation_use_discount'][ $post_id ] ) ? 'yes' : 'no';
	update_post_meta( $post_id, 'elex_ppct_variation_use_discount_post_meta', $use_discount );
	...
	update_post_meta( $post_id, 'elex_ppct_variation_add_prefix', $custom_field_prefix );
